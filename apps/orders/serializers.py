...
class OrderItemSerializer(serializers.ModelSerializer):
    ...
    def validate(self, validated_data):
        ...
        if not request:
            raise serializers.ValidationError("The request context must be passed")
        if request.method in ["PUT", "PATCH"]:
            raise serializers.ValidationError({
            "Error": "Cannot update an order item once created. Cancel and create another."
            })
        user = request.user
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if request.method in ["PUT", "PATCH"]:
            raise serializers.ValidationError({
            "Error": "Cannot update an order once created. Cancel and create another."
            })
        if not order_items:
            raise serializers.ValidationError({
